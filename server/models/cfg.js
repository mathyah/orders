class Cfg {
	...
	getAllRegi() {
		let sql = 'SELECT * FROM "CFG"."REGI" ORDER BY "ID"';
		let query = {
			text: sql
		};
	...
	getAllCom() {
		let sql = 'SELECT * FROM "CFG"."COM" ORDER BY "ID"';
		let query = {
			text: sql
		};
		return postgres.query(query);
	...
	getComById(id) {
		let sql = 'SELECT * FROM "CFG"."COM" WHERE "ID" = $1';
		let query = {
			text: sql,
			values: [id]
		};
	...
	getComByRegiId(id) {
		let sql = 'SELECT "CFG"."COM".* FROM "CFG"."COM" INNER JOIN "CFG"."ASI_REGI_COM" ON "CFG"."COM"."ID" = "CFG"."ASI_REGI_COM"."COM_ID" WHERE "CFG"."ASI_REGI_COM"."REGI_ID" = $1 GROUP BY "CFG"."COM"."ID"';
		let query = {
			text: sql,
			values: [id]
		};
